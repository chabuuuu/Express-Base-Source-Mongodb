...
export class BaseRepository<T> implements IBaseRepository<T> {
  ...
  async findOne(options: { filter: Partial<T>; relations?: string[] }): Promise<T | null> {
    ...
    if (relations) {
      result = await this.ormRepository.findOne(filter).populate(relations);
    } else {
      result = await this.ormRepository.findOne(filter);
    }
  ...
  async findMany(options: {
    ...
    const result = await this.ormRepository.find(
      filter as any,
      {},
      { skip: skip, take: take, sort: orderObject, populate: relations }
    );
    return result;
  }
